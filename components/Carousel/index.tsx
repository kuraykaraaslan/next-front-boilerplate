...
const DefaultCaresoulSmooth = ({ children , insert , hideButton}: { children?: ReactNode[], insert?: ReactNode, hideButton?: boolean }) => {
    ...
    return (
        <>
            <div className="group carousel w-full max-h-[400px] select-none" style={{ top: '0', left: '0' }} ref={sliderRef}>
                {children?.map((child, index) => (
                    <div id={`slide${index + 1}`} className="carousel-item relative w-full" key={index}
                        style={{ scrollSnapAlign: 'start', scrollSnapStop: 'always', zIndex: 10, position: 'relative' , display: hideButton ? 'none' : 'block' }}>
                        {child}
            ...
            <div className="flex gap-2" style={{
                zIndex: 10, position: 'relative', left: '0', right: '0', transform: 'translateY(-10%)', margin: 'auto',
