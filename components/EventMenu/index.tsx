...
const EventMenu = () => {
    ...
    const buttons = [
        ...
        {
            icon: <FontAwesomeIcon icon={faBars} className="text-2xl mt-2 h-8 w-8" />,
            text: "More",
            link: "/more"
        }
    ];
    ...
    return (
        <div className="absolute container mx-auto px-4 lg:px-8 max-w-7xl mb-8 -mt-4 hidden lg:block">
            <div className="flex items-center justify-between gap-12 max-w-3xl mx-auto">
                {buttons.map((button, index) => (
                    <button key={index} className="block p-6 h-20 bg-white text-black font-semibold rounded-lg shadow-md flex items-center justify-center
                    shadow-md hover:shadow-lg hover:bg-gray-100">
                        {button.icon}
                        <span className="flex items-center justify-center ml-4 text-2xl">
                            {button.text}
                        </span>
                    </button>
                ))}
